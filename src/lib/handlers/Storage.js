...
class ContainerChunk {
  ...
  scanChunks() {
    ...
    this.waitForChunk.forEach((zarr, x) => zarr?.forEach?.((status, z) => {
      ...
      const /** @type {Pos3D[]} */ chests = []
      const /** @type {Pos3D[]} */ others = []
  ...
  async init() {
    ...
    while (this.scanResult.chest[0]) {
      ...
      const otherHalfIndex = this.scanResult.chest.findIndex(v =>
        util.math.posEquals(v.pos, otherHalfPos) &&
      ...
      const otherHalf = this.scanResult.chest.splice(otherHalfIndex, 1)[0]
...
class StorageHandler {
  ...
  /** @readonly */
  get shulkerMachine() {
    const value = require('./ShulkerMachine')
