...
class ContainerHandler {
  ...
  get shulkerMacro() {
    const value = require('./ShulkerMacro')
   ...
   * @param {Dict} items 
   * @param {number[]} [slots] default hotbar + crafting_in + main, will cache
   */
  has(inv, items, slots) {
    if (!Array.isArray(slots))
      slots = invMapCache[inv.getType()] ??= Java.from(inv.getMap().hotbar)
        .concat(inv.getMap().input ?? inv.getMap().crafting_in ?? [])
   ...
   */
  async assert(inv, slot, asserter, timeout = 40) {
    if (typeof asserter === 'string') {
      const id = asserter
      asserter = item => item.getItemId() === id
    }
    if (!asserter(inv.getSlot(slot))) {
      while (timeout-- > 0 && !asserter(inv.getSlot(slot))) await util.waitTick()
      if (!asserter(inv.getSlot(slot))) return false
    }
    return true
  }
  ...
  openInventory() {
    return new Proxy(Player.openInventory(), {
      get(raw, key, proxy) {
        ...
        return asyncMethods.has(key) ? async (...args) => {
  ...
  /**
   * check if `inv` is inventory
   * @param {*} inv 
   * @returns {inv is Inventory | AsyncInventory}
   */
  isInventory(inv) {
    if (inv instanceof Inventory) return true
    if (inv?.raw instanceof Inventory) return true
    return false
  }

}

module.exports = new ContainerHandler
